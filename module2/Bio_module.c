...
double ** POINTS;
float *** Z_VALUES;	//array of all the z values of the input file
...
int setup(char * inputFile, char * outputDir, double * ulEastPtr, double * brEastPtr, double * ulNorthPtr, double * brNorthPtr)
{
	...
	//if((POINTS = readFile(inputFile, &NUM_POINTS, &X_POS, &Y_POS)) == NULL)	return 0;
	//if((POINTS = bufferTerraScanBinFile(inputFile, &NUM_POINTS)) == NULL) return 0;
	if((POINTS = bufferPoints(inputFile)) == NULL) return 0;
	int * blk_pos [2] = {&X_POS, &Y_POS};
	if(!extractGridPosition(inputFile, blk_pos)) return 0;
	
	printf("num points : %d \n", NUM_POINTS);
	int z;
	for(z=0;z<10;z++)
		printf("%lf %lf %lf \n", POINTS[0][z],POINTS[1][z],POINTS[2][z]);
	puts("Bio_module : calculating dimensions");
	...
	*brNorthPtr  = BLK_NORTHING;
	...
	printf("Max E : %lf N : %lf Z : %lf \n", getDoubleMax(POINTS[0], NUM_POINTS), getDoubleMax(POINTS[1], NUM_POINTS), getDoubleMax(POINTS[2], NUM_POINTS));
	printf("minE %lf\n", MIN_EASTING);
	printf("minN %lf\n", MIN_NORTHING);
	printf("blkS %d\n", BLK_SIZE);
	printf("xPos %d\n", X_POS);
	...
	printf("blkE %lf\n", BLK_EASTING);
	printf("blkN %lf\n", BLK_NORTHING);
	printf("Bio_module : init buffers");
	...
	for(j = 0; j < DIMENSIONS; j++)
	{
		for(i = 0; i < DIMENSIONS; i++)
		{
			xStart = BLK_EASTING + (OUT_RESOLUTION * i);
			yStart = BLK_NORTHING + (OUT_RESOLUTION * j);
			xEnd   = xStart + OUT_RESOLUTION;
			yEnd   = yStart + OUT_RESOLUTION;
			
			Array1D A;
			
			A = filterByArea(POINTS, NUM_POINTS, xStart, yStart, xEnd, yEnd);
			
			NUM_ALL[j][i]  = A.size;
			
			A = filterByZThreshold(A.points, A.size, Z_THRESHOLD);
			
			NUM_Z[j][i]    = A.size;
			Z_VALUES[j][i] = A.points;
			
			qsort(Z_VALUES[j][i], NUM_Z[j][i], sizeof(float), floatCompare);
		}
	}
...
static PyObject* Py_setup(PyObject * self, PyObject * args)
{
	...
	if(!PyArg_ParseTuple( args, "ssddiif", 
 ...
 * outArr[2] = 85th percentile
 * outArr[3-6] = l-moments
 * outArr[7] = mean above 85th percentile
...
static PyObject* Py_Rename(PyObject * self, PyObject * args)
{
	...
	if(!PyArg_ParseTuple( args, "ssddif", &inFile, &outDir, &minEasting, &minNorthing, &blockSize, &zThreshold))
	{
		return NULL;
	}
	...
	*/
	if((outFile = renameBinFile(inFile, outDir, minEasting, minNorthing, blockSize, zThreshold)) == NULL){
		return NULL;
	}
